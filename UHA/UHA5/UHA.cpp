...
void UHA::UHA_CreateBmsJson(void)
{
 ...
 /* int SunPowerW = (int)(mVars[VAR_MPPT_SOLAR_POWER_W]);
  ...
  double BattPowerW = mVars[VAR_BAT_POWER_W];
	int LoadPowerW = (int)(mVars[VAR_LOAD_W]);	
	int LoadPowerPct = (int)(LoadPowerW / 8000) * 100;
	double TodayDiffKwh = (mVars[VAR_SOLAR_ENERGY_TODAY_10WH]*10 - mVars[VAR_CONS_TODAY_10WH]*10);   // WH! 
 ...
 // cJSON_AddItemToObject(LiveData, "ChargingA", cJSON_CreateNumber(mVars[VAR_CHARGING_A10]/10.0));
  ...
  cJSON_AddItemToObject(LiveData, "TodayDischargingKwh", cJSON_CreateNumber(mVars[VAR_CONS_TODAY_10WH]/100.0));
...
cJSON_AddItemToObject(Uha, "VAR_CONS_TODAY_WH", cJSON_CreateNumber((mVars[VAR_CONS_TODAY_10WH])*10));
